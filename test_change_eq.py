...
class PlayerChangeEquipment:
    ...
    @staticmethod
    def equipment_items():
        for number,(equipment_item,attributes) in enumerate(player_equipment.equipment.items()):
    ...
    def player_change_eq_options(self):
        while True:
            try:
                ...
                if equipment_choice == ChangeEqOptions.INVENTORY.name or int(equipment_choice) == ChangeEqOptions.INVENTORY.value:
                    PlayerChangeEquipment.inventory_items()
                elif equipment_choice == ChangeEqOptions.EQUIPMENT.name or int(equipment_choice) == ChangeEqOptions.EQUIPMENT.value:
                    PlayerChangeEquipment.equipment_items()
                elif equipment_choice == ChangeEqOptions.CHANGE.name or int(equipment_choice) == ChangeEqOptions.CHANGE.value:
