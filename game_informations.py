...
def your_equipment():
    ...
    while True:
        ...
        if if_use_item == "Y":
            {item_name: item_count for (item_name, item_count) in equipment_name_count}
                ...
                if range(sum(GameAttributes.pocket.values())) == 0:
                    ...
                    if what_to_use == "ManaPotion":
                        ...
                        if player.health < player.max_health:
                            ...
                            print(f"{100 * '='}")
                            print(
                                f"Your health is maximal {player.health}/{player.max_health}".center(
                                    100
                                )
                            )
...
def informations(info):
    ...
    if info == "INFO":
        clearConsole()
        sorted(GameAttributes.player_chests.items(), key=lambda x: x[1], reverse=True)
        print(f'{100*"="}')
        ...
        print(
            f"Statistic: Health:{player.getHealth()}/{player.getMaxHealth()},Mana:{player.getMana()}/{player.getMaxMana()},Stamina:{player.getStamina()}/{player.getMaxStamina()},Attack:{player.getMinAttack()}/{player.getMaxAttack()},Defence:{player.getDefence()},Magic:{player.getMagic()},Lucky:{player.getLucky()}"
        )
