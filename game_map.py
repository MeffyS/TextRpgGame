        ...
        while True:
            if move == 'U':
                if player_backpack.coins > player_informations.game_floor * 1000:
                    up_floor = input(f"Do you want unlock up floor for {player_informations.game_floor * 1000} coins.[Y][Q] ")
                    if up_floor == 'Y':
                    ...
                    print(f"You dont have a coins {player_backpack.coins}/{player_informations.game_floor * 1000} to move on upper floor")
                    break
            break

    def coordinates():
        print(f'NORTH: [ {GameMoves.north} ] || FLOOR:{player_informations.game_floor}:FLOOR || [ {GameMoves.south} ] :SOUTH')
